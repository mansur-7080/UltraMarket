...
export class MultiLayerCache {
  ...
  constructor(
    ...
    this.memoryCache.on('set', (key: string, _value: any) => {
      this.cacheStats.sets++;
      logger.debug('🎯 Memory cache set', { key: this.sanitizeKey(key) });
    });
    
    this.memoryCache.on('del', (key: string, _value: any) => {
...
export class PerformanceMonitor {
  ...
  recordRequestEnd(req: Request, res: Response): void {
    if ((req as any).startTime) {
      const duration = Number(process.hrtime.bigint() - (req as any).startTime) / 1000000; // Convert to ms
      ...
      try {
        ...
        res.json = function(body: any) {
          ...
          if (res.statusCode < 400) {
            ...
            optimizer.cache.set(cacheKey, body, ttl).catch((error: any) => {
