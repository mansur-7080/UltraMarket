...
try {
    ...
    console.warn('Prisma not available, using fallback implementation');
...
const logger = (0, logger_replacement_1.createLogger)('email-service');
class EmailService {
    transporter;
    config;
    prisma;
     ...
     */
    async sendOrderConfirmationEmail(email, firstName, orderNumber, orderDetails) {
     ...
     */
    async sendOrderStatusUpdateEmail(email, firstName, orderNumber, status, trackingNumber) {
     ...
     */
    async sendPaymentConfirmationEmail(email, firstName, orderNumber, amount, paymentMethod) {
     ...
     */
    async sendSecurityAlertEmail(email, firstName, alertType, deviceInfo) {
     ...
     */
    getOrderConfirmationEmailTemplate(firstName, orderNumber, orderDetails) {
        return {
     ...
     */
    getOrderStatusUpdateEmailTemplate(firstName, orderNumber, status, trackingNumber) {
        return {
     ...
     */
    getPaymentConfirmationEmailTemplate(firstName, orderNumber, amount, paymentMethod) {
        return {
     ...
     */
    getSecurityAlertEmailTemplate(firstName, alertType, deviceInfo) {
        return {
    ...
    async cleanupExpiredTokens() {
        try {
            ...
            this.logger.info('Expired tokens cleaned up successfully');
        }
        catch (error) {
