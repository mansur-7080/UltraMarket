...
exports.apiDocumentation = exports.defaultAPIConfig = exports.UltraProfessionalAPIDocumentation = void 0;
...
class UltraProfessionalAPIDocumentation {
    config;
    services = new Map();
    generatedSpecs = new Map();
    validationResults = new Map();
    constructor(config) {
        this.config = config;
        ultra_professional_logger_1.logger.info('🚀 Ultra Professional API Documentation initialized', {
            title: config.title,
     ...
     */
    enhanceDescription(baseDescription) {
        return `
${baseDescription}
...
`;
     ...
     */
    generateJavaScriptSample(endpoint) {
...
`.trim();
     ...
     */
    generateServiceDocumentation(serviceName) {
        ...
        const spec = {
            ...
            tags: this.config.tags.filter(tag => serviceDoc.endpoints.some(endpoint => endpoint.tags.includes(tag.name)))
        };
     ...
     */
    generateServicePaths(serviceDoc) {
        const paths = {};
        serviceDoc.endpoints.forEach((endpoint) => {
            const pathKey = endpoint.path;
            if (!paths[pathKey]) {
     ...
     */
    async exportDocumentation(format = 'json', outputPath) {
        ...
        ultra_professional_logger_1.logger.info('📄 API documentation exported', {
            format,
            fileName,
            size: content.length
        });
    ...
    validateSpecification(spec) {
        ...
        // Validate paths
        Object.entries(specToValidate.paths || {}).forEach(([path, pathItem]) => {
            if (!path.startsWith('/')) {
     ...
     */
    async generateSDKs(outputDir) {
        const spec = this.generateOpenAPISpec();
        // This would integrate with OpenAPI generators
        ultra_professional_logger_1.logger.info('🛠️ Generating SDKs', {
            outputDir,
...
`;
            await fs.promises.writeFile(path.join(langDir, 'README.md'), readmeContent, 'utf8');
        }
    }
    /**
...
// =================== GLOBAL INSTANCE ===================
exports.apiDocumentation = new UltraProfessionalAPIDocumentation(exports.defaultAPIConfig);
exports.default = UltraProfessionalAPIDocumentation;
//# sourceMappingURL=ultra-professional-api-documentation.js.map