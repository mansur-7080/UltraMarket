...
exports.ultraSecureJWT = exports.productionJWTConfig = exports.UltraSecureJWTManager = void 0;
 ...
 */
class UltraSecureJWTManager extends events_1.EventEmitter {
    ...
    initialize() {
        ...
        ultra_professional_logger_1.logger.info('🔐 Ultra Secure JWT Manager initialized', {
    ...
    async generateTokens(payload, options = {}) {
        try {
            ...
            ultra_professional_logger_1.logger.error('❌ Token generation failed', error, {
                userId: payload.userId,
                ipAddress: options.ipAddress
            });
            this.logSecurityEvent({
     ...
     */
    async verifyToken(token, tokenType = 'access', context = {}) {
        const securityWarnings = [];
        try {
            // Check token blacklist
            if (this.config.enableTokenBlacklisting && this.blacklistedTokens.has(token)) {
                ultra_professional_logger_1.logger.warn('🚫 Blacklisted token access attempt', {
                    tokenType,
            ...
            if (error.name === 'JsonWebTokenError') {
                ultra_professional_logger_1.logger.warn('🚫 Invalid token verification attempt', {
                    error: error.message,
                    ipAddress: context.ipAddress
                });
                this.logSecurityEvent({
            ...
            ultra_professional_logger_1.logger.error('❌ Token verification error', error);
            return {
                valid: false,
    ...
    async refreshTokens(refreshToken, context = {}) {
        try {
            ...
            ultra_professional_logger_1.logger.error('❌ Token refresh failed', error);
            this.logSecurityEvent({
                type: 'security_violation',
                userId: 'unknown',
                ipAddress: context.ipAddress || 'unknown',
    ...
    blacklistToken(token) {
        if (!this.config.enableTokenBlacklisting)
            return;
        this.blacklistedTokens.set(token, new Date());
        this.metrics.tokensBlacklisted++;
        ultra_professional_logger_1.logger.info('🚫 Token blacklisted', {
    ...
    async revokeUserSessions(userId) {
        ...
        ultra_professional_logger_1.logger.info('🚪 All user sessions revoked', {
            userId,
     ...
     */
    async generateMFASetup(userId, userEmail) {
        try {
            ...
            ultra_professional_logger_1.logger.info('🔐 MFA secret generated', {
                userId,
            ...
            ultra_professional_logger_1.logger.error('❌ MFA setup generation failed', error);
    ...
    verifyMFAToken(userId, token, secret) {
        try {
            ...
            ultra_professional_logger_1.logger.error('❌ MFA verification failed', error);
            return false;
        }
    }
    /**
        ...
        return async (req, res, next) => {
            try {
                ...
                if (options.requiredPermissions) {
                    const hasAllPermissions = options.requiredPermissions.every(permission => payload.permissions.includes(permission));
                ...
                ultra_professional_logger_1.logger.error('❌ Authentication middleware error', error);
                res.status(500).json({
                    success: false,
                    error: {
     ...
     */
    async evictOldestSession(userId) {
        const userSessionIds = this.userSessions.get(userId);
        if (!userSessionIds)
            return;
        ...
        if (oldestSessionId) {
            ...
            ultra_professional_logger_1.logger.info('🚪 Oldest session evicted', {
                userId,
    ...
    logSecurityEvent(event) {
        ...
        ultra_professional_logger_1.logger.security(`Security event: ${event.type}`, {
            userId: event.userId,
    ...
    startCleanupInterval() {
        ...
        ultra_professional_logger_1.logger.info('🧹 Cleanup interval started');
    }
    /**
    ...
    startSecretRotation() {
        ...
        ultra_professional_logger_1.logger.info('🔄 Secret rotation interval started', {
    ...
    performCleanup() {
        ...
        if (expiredSessions > 0 || expiredTokens > 0) {
            ultra_professional_logger_1.logger.debug('🧹 Cleanup completed', {
                expiredSessions,
                expiredTokens,
    ...
    rotateSecret() {
        ...
        ultra_professional_logger_1.logger.info('🔄 Secret rotated', {
            newVersion,
    ...
    setupGracefulShutdown() {
        const gracefulShutdown = () => {
            ultra_professional_logger_1.logger.info('🚪 JWT Manager shutting down...');
            ...
            ultra_professional_logger_1.logger.info('✅ JWT Manager shutdown complete');
        };
...
exports.UltraSecureJWTManager = UltraSecureJWTManager;
/**
 * Production-optimized JWT configuration
 */
exports.productionJWTConfig = {
    accessTokenSecret: process.env.JWT_ACCESS_SECRET || crypto_1.default.randomBytes(64).toString('hex'),
    refreshTokenSecret: process.env.JWT_REFRESH_SECRET || crypto_1.default.randomBytes(64).toString('hex'),
 ...
 */
exports.ultraSecureJWT = UltraSecureJWTManager.getInstance(exports.productionJWTConfig);
/**
 * Helper function to get JWT manager instance
 */
function getJWTManager() {
    return exports.ultraSecureJWT;
}
//# sourceMappingURL=ultra-secure-jwt-manager.js.map