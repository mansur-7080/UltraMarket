...
const perfLogger = (0, logger_replacement_1.createLogger)('performance-optimizer');
class AdvancedPerformanceOptimizer {
    prisma;
    redis;
    config;
    queryTracker = new Map();
    constructor(prisma, redisClient, config) {
        this.prisma = prisma;
        this.redis = redisClient;
        this.config = config;
    }
    // ✅ N+1 Query Optimization
    async getProductsWithRelations(filters = {}) {
    ...
    async getCachedData(key, fetchFunction, ttl = this.config.redis.defaultTTL) {
    ...
    async batchCreateProducts(productsData) {
        ...
        try {
            ...
            for (let i = 0; i < productsData.length; i += batchSize) {
                ...
                try {
                    const batchResults = await this.prisma.$transaction(batch.map(productData => this.prisma.product.create({
                        data: productData,
                        include: {
                            vendor: true,
                            categories: true
    ...
    async optimizeQuery(queryName, queryFunction, options = {}) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Query timeout')), options.timeout || 30000));
    ...
    async monitorDatabasePerformance() {
        ...
        try {
            ...
            const [connectionMetrics, queryMetrics, cacheMetrics] = await Promise.all([
    ...
    buildProductOrderBy(sortBy, sortOrder = 'desc') {
        switch (sortBy) {
            case 'price':
    ...
    trackQuery(name, duration, resultCount) {
...
exports.default = AdvancedPerformanceOptimizer;
/**
 * USAGE EXAMPLES:
 *
 ...
 */ 
//# sourceMappingURL=advanced-performance-optimizer.js.map