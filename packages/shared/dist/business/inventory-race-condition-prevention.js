...
exports.safePurchase = exports.inventoryRaceConditionPrevention = exports.InventoryRaceConditionPrevention = void 0;
const logger_1 = require("../logging/logger");
/**
 * Inventory Race Condition Prevention Manager
 * Uses database-level row locking and atomic transactions
 */
class InventoryRaceConditionPrevention {
    static instance;
    activePurchases = new Map(); // productId -> Set<userId>
     ...
     */
    async attemptPurchase(attempt, dbTransaction // Generic database transaction
    ) {
        ...
        try {
            ...
            const inventory = await this.acquireInventoryLock(attempt, dbTransaction);
            if (!inventory) {
                return {
                    success: false,
            ...
            const reservation = await this.createInventoryReservation(attempt, inventory, dbTransaction);
            // Step 4: Update inventory atomically
            const updatedInventory = await this.updateInventoryQuantity(attempt, inventory, dbTransaction);
            logger_1.logger.info('Purchase successful with race condition prevention', {
            ...
            logger_1.logger.error('Purchase attempt failed', {
                userId: attempt.userId,
                productId: attempt.productId,
     ...
     */
    async acquireInventoryLock(attempt, dbTransaction) {
        const query = `
      SELECT 
        product_id,
        variant_id,
     ...
     */
    async acquireInventoryLockMongo(attempt, mongoSession) {
        const filter = {
            productId: attempt.productId,
     ...
     */
    checkAvailability(inventory, requestedQuantity) {
     ...
     */
    async createInventoryReservation(attempt, inventory, dbTransaction) {
     ...
     */
    async updateInventoryQuantity(attempt, inventory, dbTransaction) {
        const updateQuery = `
      UPDATE inventory 
      SET 
     ...
     */
    async updateInventoryQuantityMongo(attempt, inventory, mongoSession, originalVersion) {
        const filter = {
            productId: attempt.productId,
            ...(attempt.variantId && { variantId: attempt.variantId }),
            warehouseId: inventory.warehouseId,
     ...
     */
    generateLockKey(productId, variantId) {
        return variantId ? `${productId}:${variantId}` : productId;
    }
    /**
     * Check if user has active purchase
     */
    isUserActivePurchasing(lockKey, userId) {
     ...
     * Register active purchase
     */
    registerActivePurchase(lockKey, userId) {
     ...
     */
    unregisterActivePurchase(lockKey, userId) {
        const activeUsers = this.activePurchases.get(lockKey);
        if (activeUsers) {
     ...
     */
    async cleanupExpiredReservations(dbTransaction) {
        const cleanupQuery = `
    ...
    `;
        ...
        if (cleanedCount > 0) {
            logger_1.logger.info('Cleaned up expired inventory reservations', { count: cleanedCount });
        }
        return cleanedCount;
    }
    /**
...
exports.InventoryRaceConditionPrevention = InventoryRaceConditionPrevention;
// Export singleton instance
exports.inventoryRaceConditionPrevention = InventoryRaceConditionPrevention.getInstance();
/**
 * Utility function for safe inventory purchase
 */
const safePurchase = async (attempt, dbTransaction) => {
    return await exports.inventoryRaceConditionPrevention.attemptPurchase(attempt, dbTransaction);
};
exports.safePurchase = safePurchase;
exports.default = exports.inventoryRaceConditionPrevention;
//# sourceMappingURL=inventory-race-condition-prevention.js.map